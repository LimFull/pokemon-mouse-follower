...
<p align="center">
  <img src="docs/assets/follower.gif" width="480" alt="마우스 커서를 따라다니는 꼬부기">
</p>
...
- 🎮 **키우기 모드** — 스타터 선택, 야생 조우, 화면 위 자동 배틀, 경험치/진화, 포획, 6마리 파티와 가방
...
팔로워를 직접 키우는 포켓몬으로 바꿔보세요. 설정에서 **키우기 모드**를 켜고 클래식 스타터 6마리 중 하나를 고르면 설정 옆에 파티 패널이 나타납니다.

<p align="center">
  <img src="docs/assets/raising.png" width="741" alt="키우기 모드 파티 패널이 열린 설정 창: HP 바가 있는 파티 4마리, 내보내기/집어넣기 버튼, 가방과 키우기 설정">
</p>

- 🌱 **스타터** — 1·2세대 클래식 스타터 6마리 중 하나로 시작
- ⚔️ **야생 조우** — 야생 포켓몬이 데스크톱에 출현해 배회하고, 다가가면 오버레이 위에서 자동 배틀이 펼쳐집니다 (타입 상성, 기술 이펙트, 상태이상 — 수면·화상·마비·얼음·혼란 등)
- ✨ **경험치 → 레벨 → 진화** — 승리하면 경험치를 얻고, 레벨업하면 머리 위에 "Level Up!" 표시가 뜨며, 새 기술을 배우고(4개일 땐 교체 선택), 진화는 클래식 화이트아웃 연출로 재생됩니다
- 🔴 **포획** — 가방에서 포획을 켜면 약해지거나 상태이상에 걸린 야생에게 몬스터볼이 자동으로 날아갑니다. 최대 6마리 파티, 따라다닐 포켓몬 교체 가능
- 🎒 **가방·아이템** — 아이템이 데스크톱에 떨어지고 밟고 지나가면 획득. 회복약·볼·진화 아이템을 패널에서 사용
- 💤 **휴식과 회복** — 다친 멤버는 배틀 밖에서 서서히 회복되고, 팔로워가 멈춘 커서 옆에서 잠들면 4배 빨라지며, 자정에는 파티 전체가 완전 회복됩니다 (기절한 멤버도 그때 부활)
- 🏃 **본가식 도망 타이밍** — 배틀 중 집어넣기는 진행 중인 턴이 끝나야 실행되고, 입은 대미지(와 상태이상)는 그대로 가져갑니다

진행 상황은 `~/Library/Application Support/PokemonMouseFollower/raising.json`에 저장됩니다.
