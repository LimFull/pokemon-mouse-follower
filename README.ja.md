...
<p align="center">
  <img src="docs/assets/follower.gif" width="480" alt="マウスカーソルを追いかけるゼニガメ">
</p>
...
- 🎮 **育成モード** — 御三家選択、野生との遭遇、画面上のオートバトル、経験値／進化、捕獲、6匹のパーティとバッグ
...
- **macOS**: macOS 13（Ventura）以降、Apple Silicon / Intel の両対応（universal ビルド）
- **Windows**: Windows 10 1809 以降（x64）
- ソースからビルドする場合のみ: macOS は Xcode Command Line Tools（`xcode-select --install`）、Windows は [Swift ツールチェーン](https://www.swift.org/install/windows/) + VS Build Tools

## インストール — macOS（開発者ツール不要）
...
[Releases](https://github.com/LimFull/pokemon-mouse-follower/releases/latest) から `PokemonMouseFollower-Setup.exe`（インストーラー）または `PokemonMouseFollower-<version>-windows.zip`（ポータブル）をダウンロードします。

- コード署名がないため SmartScreen の警告が表示されることがあります — **詳細情報 → 実行** を選んでください。
- インストールはユーザー単位（管理者権限不要）で、スタートメニューに登録されます。アプリはシステムトレイに常駐します。
- 設定 / 一時停止 / アップデート確認はトレイアイコンの右クリックメニューにあります。
- 注意: *排他的フルスクリーン* のゲーム上にはオーバーレイを表示できません（ボーダーレスウィンドウは可）。育成モードは今後の Windows リリースで対応予定です。
...
フォロワーを自分で育てるポケモンに変えられます。設定で**育成モード**をオンにし、クラシックな御三家6匹から1匹選ぶと、設定の隣にパーティパネルが現れます。

<p align="center">
  <img src="docs/assets/raising.png" width="741" alt="育成モードのパーティパネルが開いた設定ウィンドウ: HPバー付きのパーティ4匹、繰り出し／引っ込めボタン、バッグと育成設定">
</p>

- 🌱 **御三家** — 第1・2世代のクラシックな御三家6匹から選んでスタート
- ⚔️ **野生との遭遇** — 野生ポケモンがデスクトップに出現してうろつき、近づくとオーバーレイ上でオートバトルが始まります（タイプ相性、わざエフェクト、状態異常 — ねむり・やけど・まひ・こおり・こんらん など）
- ✨ **経験値 → レベル → 進化** — 勝つと経験値を獲得。レベルアップすると頭上に「Level Up!」が浮かび、新しいわざを覚え（4つのときは入れ替え選択）、進化はクラシックなホワイトアウト演出で再生されます
- 🔴 **捕獲** — バッグで捕獲をオンにすると、弱った・状態異常の野生にモンスターボールが自動で飛びます。最大6匹のパーティ、カーソルを追うポケモンの交代も可能
- 🎒 **バッグ・アイテム** — アイテムがデスクトップに落ち、歩いて踏むと入手。キズぐすり・ボール・進化アイテムはパネルから使用
- 💤 **休息と回復** — 傷ついたメンバーはバトル外でゆっくり回復し、フォロワーが止まったカーソルのそばで眠ると4倍速に。深夜0時にはパーティ全体が全回復します（ひんしのメンバーもそのとき復活）
- 🏃 **本家流の逃げるタイミング** — バトル中の引っ込めは進行中のターンが終わってから実行され、受けたダメージ（と状態異常）はそのまま持ち帰ります

進行状況は `~/Library/Application Support/PokemonMouseFollower/raising.json` に保存されます。
