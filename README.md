...
- 😴 **대기 → 수면 → 기상** — 일정 시간 멈춰 있으면 잠들고, 커서가 움직이면 깨어남
...
> 로그인 시 자동 실행은 **시스템 설정 → 일반 → 로그인 항목**에 앱을 추가하세요.
...
각 캐릭터 폴더에는 `Idle-Anim.png`, `Walk-Anim.png`, `Sleep-Anim.png`(있는 경우 `Wake-Anim.png`)와 프레임 크기 정보가 담긴 `AnimData.xml`이 들어 있습니다. 프레임 크기는 캐릭터마다 다르며, 앱이 `AnimData.xml`을 읽어 동적으로 슬라이스합니다.
